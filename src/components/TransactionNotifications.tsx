  ...
  return (
    <div className="relative">
      ...
      >
        ...
        {unreadCount > 0 && (
          <span className="absolute top-1 right-1 flex items-center justify-center w-4 h-4 text-[10px] font-bold text-brand-text bg-red-500 rounded-full">
      ...
      {isOpen && (
        <div className="absolute right-0 top-10 w-80 bg-brand-card border border-brand-border/40 rounded-xl shadow-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between p-3 border-b border-brand-border/40 bg-brand-bg">
            <h3 className="text-sm font-bold text-brand-text">Notifications</h3>
            <button onClick={onMarkAllRead} className="text-[10px] text-brand-accent hover:text-emerald-400">Mark all read</button>
          </div>
          
          <div className="max-h-96 overflow-y-auto">
            {notifications.length === 0 ? (
              <div className="p-8 text-center text-brand-text-dim text-xs">No notifications</div>
            ) : (
              notifications.slice(0, 10).map((n) => (
                <div key={n.id} className={`p-3 border-b border-emerald-950/20 ${!n.read ? 'bg-emerald-950/10' : ''}`}>
                  <div className="flex justify-between items-start">
                    <h4 className="text-xs font-bold text-brand-text">{n.title}</h4>
                    {!n.read && <button onClick={() => onMarkRead(n.id)} className="text-[10px] text-brand-accent">Mark read</button>}
                  </div>
                  <p className="text-[10px] text-brand-text-muted mt-1">{n.message}</p>
                </div>
              ))
            )}
          </div>
          ...
          <button 
            ...
            className="w-full p-2 text-xs text-center text-brand-text-muted bg-brand-bg hover:bg-emerald-950/40"
          >
